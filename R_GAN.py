...
def main(win_size=20, step_size=1, eps=3000, n_crit=5):
    ...
    for epoch in range(1, epochs + 1):
        ...
        while len(idxs) >= batch_size:
            ...
            # grad_penalty_2 = p_coeff * torch.mean(torch.pow(torch.norm(xp_grad[0], 2, 2) - 1, 2))  # !!! norma en dim 2 <--- MAL!
            ...
            # print(f'{grad_penalty_2.item()} {grad_penalty.item()}')
        ...
        if 250 <= epoch < 1000:
            if best_D_values > np.abs(np.mean(D_z_value)) + np.abs(np.mean(D_x_value)):
                ...
                torch.save(G.state_dict(), 'C:/Users/D255728/Documents/ProyectoGAN/model/R/generator_R_' + str(K) + '_' + str(S) + '_' + str(epochs) + '_' + str(generator_learning_rate) + '_' + str(dynamic_LR) + '_' + str(n_critic) + '_best_1000' + '.pth')
        if epoch == 1000:
        ...
        if 1000 <= epoch < 2000:
            if best_D_values > np.abs(np.mean(D_z_value)) + np.abs(np.mean(D_x_value)):
                best_D_values = np.abs(np.mean(D_z_value)) + np.abs(np.mean(D_x_value))
                torch.save(G.state_dict(), 'C:/Users/D255728/Documents/ProyectoGAN/model/R/generator_R_' + str(K) + '_' + str(S) + '_' + str(epochs) + '_' + str(generator_learning_rate) + '_' + str(dynamic_LR) + '_' + str(n_critic) + '_best_2000' + '.pth')
        if epoch == 2000:
            print(f'{best_D_values=}')
            best_D_values = 100
        if 2000 <= epoch:
            if best_D_values > np.abs(np.mean(D_z_value)) + np.abs(np.mean(D_x_value)):
                best_D_values = np.abs(np.mean(D_z_value)) + np.abs(np.mean(D_x_value))
                torch.save(G.state_dict(), 'C:/Users/D255728/Documents/ProyectoGAN/model/R/generator_R_' + str(K) + '_' + str(S) + '_' + str(epochs) + '_' + str(generator_learning_rate) + '_' + str(dynamic_LR) + '_' + str(n_critic) + '_best_3000' + '.pth')
    ...
    torch.save(G.state_dict(), 'C:/Users/D255728/Documents/ProyectoGAN/model/R/generator_R_' + str(K) + '_' + str(S) + '_' + str(epochs) + '_' + str(generator_learning_rate) + '_' + str(dynamic_LR) + '_' + str(n_critic) + '.pth')
    torch.save(D.state_dict(), 'C:/Users/D255728/Documents/ProyectoGAN/model/R/discriminator_R_' + str(K) + '_' + str(S) + '_' + str(epochs) + '_' + str(generator_learning_rate) + '_' + str(dynamic_LR) + '_' + str(n_critic) + '.pth')
